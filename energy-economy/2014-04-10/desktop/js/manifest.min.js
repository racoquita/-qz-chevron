...
this.wrapper.offsetHeight;this.options.listenX&&(this.wrapperWidth=this.wrapper.clientWidth,this.options.resize?(this.indicatorWidth=Math.max(Math.round(this.wrapperWidth*this.wrapperWidth/(this.scroller.scrollerWidth||this.wrapperWidth||1)),8),this.indicatorStyle.width=this.indicatorWidth+"px"):this.indicatorWidth=this.indicator.clientWidth,this.maxPosX=this.wrapperWidth-this.indicatorWidth,"clip"==this.options.shrink?(this.minBoundaryX=-this.indicatorWidth+8,this.maxBoundaryX=this.wrapperWidth-8):(this.minBoundaryX=0,this.maxBoundaryX=this.maxPosX),this.sizeRatioX=this.options.speedRatioX||this.scroller.maxScrollX&&this.maxPosX/this.scroller.maxScrollX),this.options.listenY&&(this.wrapperHeight=this.wrapper.clientHeight,this.options.resize?(this.indicatorHeight=Math.max(Math.round(this.wrapperHeight*this.wrapperHeight/(this.scroller.scrollerHeight||this.wrapperHeight||1)),8),this.indicatorStyle.height=this.indicatorHeight+"px"):this.indicatorHeight=this.indicator.clientHeight,this.maxPosY=this.wrapperHeight-this.indicatorHeight,"clip"==this.options.shrink?(this.minBoundaryY=-this.indicatorHeight+8,this.maxBoundaryY=this.wrapperHeight-8):(this.minBoundaryY=0,this.maxBoundaryY=this.maxPosY),this.maxPosY=this.wrapperHeight-this.indicatorHeight,this.sizeRatioY=this.options.speedRatioY||this.scroller.maxScrollY&&this.maxPosY/this.scroller.maxScrollY),this.updatePosition()},updatePosition:function(){var x=this.options.listenX&&Math.round(this.sizeRatioX*this.scroller.x)||0,y=this.options.listenY&&Math.round(this.sizeRatioY*this.scroller.y)||0;this.options.ignoreBoundaries||(x<this.minBoundaryX?("scale"==this.options.shrink&&(this.width=Math.max(this.indicatorWidth+x,8),this.indicatorStyle.width=this.width+"px"),x=this.minBoundaryX):x>this.maxBoundaryX?"scale"==this.options.shrink?(this.width=Math.max(this.indicatorWidth-(x-this.maxPosX),8),this.indicatorStyle.width=this.width+"px",x=this.maxPosX+this.indicatorWidth-this.width):x=this.maxBoundaryX:"scale"==this.options.shrink&&this.width!=this.indicatorWidth&&(this.width=this.indicatorWidth,this.indicatorStyle.width=this.width+"px"),y<this.minBoundaryY?("scale"==this.options.shrink&&(this.height=Math.max(this.indicatorHeight+3*y,8),this.indicatorStyle.height=this.height+"px"),y=this.minBoundaryY):y>this.maxBoundaryY?"scale"==this.options.shrink?(this.height=Math.max(this.indicatorHeight-3*(y-this.maxPosY),8),this.indicatorStyle.height=this.height+"px",y=this.maxPosY+this.indicatorHeight-this.height):y=this.maxBoundaryY:"scale"==this.options.shrink&&this.height!=this.indicatorHeight&&(this.height=this.indicatorHeight,this.indicatorStyle.height=this.height+"px")),this.x=x,this.y=y,this.scroller.options.useTransform?this.indicatorStyle[utils.style.transform]="translate("+x+"px,"+y+"px)"+this.scroller.translateZ:(this.indicatorStyle.left=x+"px",this.indicatorStyle.top=y+"px")},_pos:function(x,y){0>x?x=0:x>this.maxPosX&&(x=this.maxPosX),0>y?y=0:y>this.maxPosY&&(y=this.maxPosY),x=this.options.listenX?Math.round(x/this.sizeRatioX):this.scroller.x,y=this.options.listenY?Math.round(y/this.sizeRatioY):this.scroller.y,this.scroller.scrollTo(x,y)},fade:function(val,hold){if(!hold||this.visible){clearTimeout(this.fadeTimeout),this.fadeTimeout=null;var time=val?250:500,delay=val?0:300;val=val?"1":"0",this.wrapperStyle[utils.style.transitionDuration]=time+"ms",this.fadeTimeout=setTimeout(function(val){this.wrapperStyle.opacity=val,this.visible=+val}.bind(this,val),delay)}}},IScroll.utils=utils,"undefined"!=typeof module&&module.exports?module.exports=IScroll:window.IScroll=IScroll}(window,document,Math);var App=function(){var that=this,slider=null,current=0,timeouts=[],intervals=[];this.on=function(){slider=new IScroll("#slider",{scrollY:!1,scrollX:!0,snap:"li",momentum:!1,disableMouse:!0,disablePointer:!0,bounceEasing:"quadratic",deceleration:.01}),setTimeout(function(){c1=document.getElementById("sc1"),c1.width=720,c1.height=175,ctx1=c1.getContext("2d"),that.initText1(),that.animateText1(),c2=document.getElementById("sc2"),c2.width=640,c2.height=45,ctx2=c2.getContext("2d"),c3=document.getElementById("sc3"),c3.width=514,c3.height=87,ctx3=c3.getContext("2d"),c4=document.getElementById("sc4"),c4.width=804,c4.height=56,ctx4=c4.getContext("2d")},250),slider.on("scrollEnd",function(){var page=slider.currentPage.pageX;1==page&&(that.initText2(),timeouts[0]=setTimeout(function(){$(".map-people").show().addClass("bounceIn"),that.animateText2()},250)),2==page&&(that.initText3(),timeouts[1]=setTimeout(function(){$(".man").show().addClass("bounceIn"),timeouts[2]=setTimeout(function(){$(".men").show().addClass("bounceIn"),that.animateText3()},250)},250)),3==page&&(that.initText4(),timeouts[3]=setTimeout(function(){$(".slide:nth-child(4) .heading, #sc4").show().addClass("bounceIn"),timeouts[4]=setTimeout(function(){$(".cta-inner").css("display","block").addClass("bounceIn"),timeouts[5]=setTimeout(function(){that.animateText4()},500)},250)},250))}),$(".arrow-right").on("click",function(){3>current&&(slider.goToPage(current+1,0,250,IScroll.utils.easeIn),current++,that.handleChange())}),$(".arrow-left").on("click",function(){current>0&&(slider.goToPage(current-1,0,250,IScroll.utils.easeIn),current--,that.handleChange())}),$(".one").on("click",function(){$(".info-one").toggle()}),$(".two").on("click",function(){$(".info-two").toggle()}),$(".slide:nth-child(1) .heading, #sc1").show().addClass("bounceInReverse")},this.off=function(){slider.destroy(),slider=null},this.handleChange=function(){var slides=$(".slide").css("visibility","visible");3==current?$(".arrow-right").hide():$(".arrow-right").show(),0==current?$(".arrow-left").hide():$(".arrow-left").show(),1==current||2==current?$(".cta").show():$(".cta").hide(),$(".dot").removeClass("active"),$(".dot:nth-child("+(current+1)+")").addClass("active"),$(".info-one, .info-two").hide(),timeouts[5]=setTimeout(function(){$.each(slides,function(i,slide){i!=current&&$(slide).css("visibility","hidden")})},1e3)},this.initText1=function(){ctx1.restore(),ctx1.font="53px interstate_cond_monobold",ctx1.fillStyle="#4f4e4c",ctx1.fillText("WHICH INDUSTRY IS",0,40),ctx1.fillText("CREATING AMERICAN JOBS",0,100),ctx1.fillText("AND",555,100),ctx1.fillText("STRENGTHENING THE ECONOMY?",0,160)},this.animateText1=function(){var i=0;intervals[0]=setInterval(function(){that.changeText1(i),i++,100==i&&clearInterval(intervals[0])},10)},this.changeText1=function(i){var gradient=ctx1.createLinearGradient(0,0,c1.width,0);gradient.addColorStop(i/100,"#009dd9"),gradient.addColorStop(i/100,"#4f4e4c"),ctx1.clearRect(0,40,550,65),ctx1.fillStyle=gradient,ctx1.fillText("CREATING AMERICAN JOBS",0,100)},this.initText2=function(){ctx2.font="48px interstate_cond_monobold",ctx2.fillStyle="#4f4e4c",ctx2.fillText("THE ANSWER IS",0,40),ctx2.fillText("ENERGY",290,40)},this.animateText2=function(){var i=0;intervals[1]=setInterval(function(){that.changeText2(i),i++,100==i&&clearInterval(intervals[1])},10)},this.changeText2=function(i){var gradient=ctx2.createLinearGradient(0,0,c2.width,0);gradient.addColorStop(i/100,"#009dd9"),gradient.addColorStop(i/100,"#4f4e4c"),ctx2.clearRect(290,0,150,45),ctx2.fillStyle=gradient,ctx2.fillText("ENERGY",290,40)},this.initText3=function(){ctx3.font="78px interstate_cond_monobold",ctx3.fillStyle="#4f4e4c",ctx3.fillText("CREATES 3 MORE",0,81)},this.animateText3=function(){var i=0;intervals[2]=setInterval(function(){that.changeText3(i),i++,101==i&&clearInterval(intervals[2])},10)},this.changeText3=function(i){var gradient=ctx3.createLinearGradient(0,0,c3.width,0);gradient.addColorStop(i/100,"#009dd9"),gradient.addColorStop(i/100,"#4f4e4c"),ctx3.clearRect(0,0,514,100),ctx3.fillStyle=gradient,ctx3.fillText("CREATES 3 MORE",0,81)},this.initText4=function(){ctx4.font="48px interstate_cond_monobold",ctx4.fillStyle="#4f4e4c",ctx4.fillText("ENERGY",0,50),ctx4.fillText("DRIVES OUR ECONOMY FORWARD.",155,50)},this.animateText4=function(){var i=0;intervals[3]=setInterval(function(){that.changeText4(i),i++,100==i&&clearInterval(intervals[3])},10)},this.changeText4=function(i){var gradient=ctx4.createLinearGradient(0,0,c4.width,0);gradient.addColorStop(i/100,"#009dd9"),gradient.addColorStop(i/100,"#4f4e4c"),ctx4.clearRect(0,0,155,56),ctx4.fillStyle=gradient,ctx4.fillText("ENERGY",0,50)}};